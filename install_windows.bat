...
powershell -NoProfile -Command "& { $w = New-Object -ComObject WScript.Shell; foreach ($path in @('%DESKTOP_SHORTCUT%', '%START_MENU_SHORTCUT%')) { $shortcut = $w.CreateShortcut($path); $shortcut.TargetPath = 'python'; $shortcut.Arguments = '"%SCRIPT%" --gui'; $shortcut.WorkingDirectory = '%WORKDIR%'; $shortcut.IconLocation = '$env:SystemRoot\\System32\\shell32.dll,1'; $shortcut.Save(); } }"
